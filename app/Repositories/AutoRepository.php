...
class AutoRepository implements AvtosInterface
{
    ...
    public function getWithRecordsByUser(int $userId)
    {
        $result = DB::table('avtos')
            ...
            ->select('avtos.id', 'avtos.id_citisen', 'avtos.brand_avto', 'avtos.addit_inf', 'avtos.regis_num', 'avtos.color',
                'avtos.who_noticed', 'avtos.where_notice', 'avtos.detection_time', 'avtos.user')
            ->where('records.id_user',  $userId)
            ->get();

        return $result;
    }

    public function searchAutos(string $search): LengthAwarePaginator
    {
        $result = DB::table('avtos')
